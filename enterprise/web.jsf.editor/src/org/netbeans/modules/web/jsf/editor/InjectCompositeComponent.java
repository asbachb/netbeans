        ...
        if (result != null && result.size() > 0) {
            ...
            ParserManager.parseWhenScanFinished(Collections.singletonList(documentSource), new UserTask() { //NOI18N
                ...
                public void run(ResultIterator resultIterator) throws Exception {
                    ...
                    if (lib != null) {
                        if (!LibraryUtils.importLibrary(document, lib, prefix, jsfs.getJsfVersion().isAtLeast(JsfVersion.JSF_2_2))) { //XXX: fix the damned static prefix !!!
                    ...
                    RequestProcessor.getDefault().post(new Runnable() {
                        ...
                        public void run() {
                            ((BaseDocument) templateInstanceDoc).runAtomic(new Runnable() {
                                @Override
                                public void run() {
                                    LibraryUtils.importLibrary(templateInstanceDoc, importsMap, jsfs.getJsfVersion().isAtLeast(JsfVersion.JSF_2_2));
                                }
                            });
                            try {
